...
public class StarterGame extends GameEngine2D {
    ...
    protected void scrollEntities(){
        ...
        for (int i = 0; i < displayList.size(); i++){
            ...
            if (entity instanceof Scrollable) {
                ...
                if ( !entity.isFlaggedForGC() && entity instanceof Consumable && player.isColliding(entity)){
                    //entity.setGCFlag(true);
                    collidedWithPlayer((Consumable)entity);
   ...
   protected void gcOffscreenEntities(){
    ...
    protected void spawnEntities(){
        ...
        int numberOfEntitiesToAdd = rand.nextInt(5) + 1;
        for (int i = 0; i < numberOfEntitiesToAdd; i++){ // Generate random Y positions for the entities
            int randomY = rand.nextInt(getWindowHeight() - Avoid.AVOID_HEIGHT); // Create a new entity based on a random chance
            Entity entityToAdd;
            int chance = rand.nextInt(100);
            if (chance < 50){ // 50% chance to spawn an Avoid
                entityToAdd = new Avoid(getWindowWidth(), randomY);
            }
            else if (chance < 80){ // 30% chance to spawn a Collect
                entityToAdd = new Collect(getWindowWidth(), randomY);
            }
            else if (chance < 90){ // 10% chance to spawn a RareAvoid
                entityToAdd = new RareAvoid(getWindowWidth(), randomY);
            }
            else{ // 10% chance to spawn a RareCollect
                entityToAdd = new RareCollect(getWindowWidth(), randomY);
            } // Ensure the entity does not overlap with existing entities
    ...
    protected void postgame(){
        ...
        if (player.getHP() <= 0)
            super.setTitleText("GAME OVER - You Lost, try agian!");
        //if (this.score >= SCORE_TO_WIN && player.getHP() > 0)
        else
            super.setTitleText("GAME OVER - You Won, Congratulations!");
    }
